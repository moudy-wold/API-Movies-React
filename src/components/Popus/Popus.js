...
export default function Popus(props) {
    ...
    getData();
    // For Check Search Input Empty Or No
    ...
    var scroll = window;
    // For Send ScrollTop Data To NavBar
    useEffect(() => {
    ...
    return (
        <div className='popus main' ref={carousel}>
            {searchInputValue == "" ?
                <>
                    <h1 className="m-4 mb-1 mt-5">Popular Movies</h1>
                    <motion.div className="films overflow-hidden" whileTap={{ cursor: "grabbing" }}>
                        <motion.div className="film search d-flex" drag="x" dragConstraints={{ right: 0, left: -4810 }}>
                            {data.map(item => (
